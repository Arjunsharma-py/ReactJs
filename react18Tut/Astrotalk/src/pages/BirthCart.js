...
function BirthCart() {
  ...
  const handleForcast = async () => {
    try {
      ...
      const BirthcartData = await API({
        url: `${apiURl.birthcart}`,

        method: "POST",
        body: payload,
      });
      //   const ForcastData = await fetch(
      //     "http://192.168.2.171:8000/astroforecast",
      //     {
      //       method: "POST",
      ...
      console.log("response", BirthcartData);
      toast.success(BirthcartData?.message);
      setPrediction(BirthcartData?.response);
      setLoading(false);
    } catch (error) {
      console.log(error);
    }
        ...
        <div className="container">
          <div className="body flex column a-center j-center">
            <div className="text flex column">
              ...
              >
                Information through Birth Chart
              </h1>
            </div>
            <div className="form">
              <textarea
